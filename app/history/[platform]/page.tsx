      ...
      <div className="neo-container max-w-4xl animate-fade-in-up">
        <header className="mb-12 text-center">
          ...
          <p className="neo-subtitle max-w-2xl mx-auto">
            Browse all finalized giveaway draws and results for{" "}
            {platform === "x" ? (
              "X / Twitter"
            ) : (
        ...
        <div className="space-y-4">
          {loading ? (
            [...Array(3)].map((_, i) => (
              <div
                key={i}
                className="neo-card p-6 h-24 animate-pulse-slow opacity-50"
              />
            ))
          ) : draws.length === 0 ? (
            <div className="neo-card p-12 text-center">
              <p className="text-textSecondary">
                No finalized draws found for this platform yet.
              </p>
            </div>
          ) : (
            paged.map((draw) => (
              ...
              >
                <div className="neo-card p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:border-accentPrimary">
                  <div className="flex items-center gap-4">
                    <div className="h-12 w-12 shrink-0">
                      {draw.hostUsername ? (
                        <Avatar
                          username={draw.hostUsername}
                          src={draw.hostAvatarUrl}
                          size="lg"
                        />
                      ) : (
                        <div className="w-12 h-12 rounded-full bg-bgElevated border border-borderStrong flex items-center justify-center text-textMuted">
                          <FaCube />
                        </div>
                      )}
                    </div>
                    <div>
                      ...
                      <p className="text-sm text-textSecondary">
                        {new Date(draw.createdAt).toLocaleDateString()} at{" "}
                  ...
                  <div className="flex flex-col sm:items-end gap-3 mt-4 sm:mt-0">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-accentPrimary/10 border border-accentPrimary/20 text-accentPrimary font-bold text-xs uppercase tracking-wide">
                        <FaTrophy />{" "}
                        {
                          draw.winners.filter((w) => w.status !== "failed")
                            .length
                        }{" "}
                        {draw.winners.filter((w) => w.status !== "failed")
                          .length === 1
                    ...
                    {draw.winners.length > 0 && (
                      <div className="flex -space-x-2">
                        ...
                        {draw.winners.length > 3 && (
                          <div
                            className="w-8 h-8 rounded-full border-2 border-bgBase bg-bgElevated text-[10px] font-bold text-textSecondary flex items-center justify-center relative shadow-sm"
                            style={{ zIndex: 0 }}
                          >
                            +{draw.winners.length - 3}
                          </div>
