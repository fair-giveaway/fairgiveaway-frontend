...
export default function FinalizedSession({
  data,
  drawId,
}: {
  data: GiveawayDoc;
  ...
  const generateShareText = () => {
    ...
    const primary = data.winners.filter(
      (w) => w.type === "primary" && w.status !== "failed",
    );
  ...
  const shareToXAsQuote = () => {
    ...
    const quoteUrl = encodeURIComponent(
      `https://x.com/i/status/${data.tweetId}`,
    );
    window.open(
      `https://x.com/intent/tweet?text=${text}&url=${quoteUrl}`,
      "_blank",
    );
  };
  ...
  const shareToXAsComment = () => {
    const text = generateShareText();
    window.open(
      `https://x.com/intent/tweet?in_reply_to=${data.tweetId}&text=${text}`,
      "_blank",
    );
  };

  const primaryWinners = data.winners.filter(
    (w) => w.type === "primary" && w.status !== "failed",
  );
  const secondaryWinners = data.winners.filter(
    (w) => w.type === "secondary" && w.status !== "failed",
  );
  ...
  const hasEngagement =
    ...
    (data.engagementTasks.mustLike ||
      data.engagementTasks.mustComment ||
      data.engagementTasks.mustFollow ||
      data.engagementTasks.mustExternal);
  const hasAntiBot =
    data.antiBotFilters &&
    (data.antiBotFilters.mustPfp ||
      data.antiBotFilters.mustBio ||
      data.antiBotFilters.mustAge ||
      data.antiBotFilters.mustActivity);
      ...
      {hasNewFeatures ? (
              ...
              <ul className="space-y-2 text-sm text-textSecondary">
                {data.engagementTasks?.mustLike && (
                  <li className="flex items-center gap-2">
                    <FaCheck className="text-emerald-500" /> Must Like Target
                    Post
                  </li>
                )}
                {data.engagementTasks?.mustComment && (
                  <li className="flex items-center gap-2">
                    <FaCheck className="text-emerald-500" /> Must Comment
                  </li>
                )}
                {data.engagementTasks?.mustFollow &&
                  data.engagementTasks.followUsernames &&
                  data.engagementTasks.followUsernames.length > 0 && (
                    <li className="flex items-center gap-2">
                      <FaCheck className="text-emerald-500" /> Follow:{" "}
                      {data.engagementTasks.followUsernames
                        .map((u) => `@${u}`)
                        .join(", ")}
                    </li>
                  ...
                  data.engagementTasks.externalUrl && (
                    <li className="flex flex-col gap-1.5 mt-2">
                      <div className="flex items-center gap-2">
                        <FaCheck className="text-emerald-500" /> External Post
                        Interaction
                      </div>
                      <a
                        href={data.engagementTasks.externalUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="ml-6 text-xs font-semibold text-accentPrimary hover:underline break-all"
                      >
                        {data.engagementTasks.externalUrl}
                      </a>
                      <div className="ml-6 mt-0.5 flex flex-wrap gap-1.5">
                        {data.engagementTasks.extMustLike && (
                          <span className="text-[10px] uppercase font-bold bg-borderStrong text-textPrimary px-2 py-0.5 rounded">
                            Like
                          </span>
                        )}
                        {data.engagementTasks.extMustRepost && (
                          <span className="text-[10px] uppercase font-bold bg-borderStrong text-textPrimary px-2 py-0.5 rounded">
                            Repost
                          </span>
                        )}
                        {data.engagementTasks.extMustComment && (
                          <span className="text-[10px] uppercase font-bold bg-borderStrong text-textPrimary px-2 py-0.5 rounded">
                            Comment
                          </span>
                        )}
                        {data.engagementTasks.extMustQuote && (
                          <span className="text-[10px] uppercase font-bold bg-borderStrong text-textPrimary px-2 py-0.5 rounded">
                            Quote
                          </span>
                        )}
          ...
          {hasAntiBot && (
            <div>
              ...
              <ul className="space-y-2 text-sm text-textSecondary">
                {data.antiBotFilters?.mustPfp && (
                  <li className="flex items-center gap-2">
                    <FaCheck className="text-emerald-500" /> Profile Picture &
                    Banner
                  </li>
                )}
                {data.antiBotFilters?.mustBio && (
                  <li className="flex items-center gap-2">
                    <FaCheck className="text-emerald-500" /> Custom Bio Text
                  </li>
                )}
                {data.antiBotFilters?.mustAge && (
                  <li className="flex items-center gap-2">
                    <FaCheck className="text-emerald-500" /> Min. Account Age:{" "}
                    {data.antiBotFilters.minMonths} months
                  </li>
                )}
                {data.antiBotFilters?.mustActivity && (
                  <li className="flex items-center gap-2">
                    <FaCheck className="text-emerald-500" /> Min. Post Count:{" "}
                    {data.antiBotFilters.minPosts} posts
                  </li>
                )}
              </ul>
            </div>
        ...
        <div className="flex flex-wrap gap-2">
          {data.enabledFeatures!.map((feat, i) => (
            <span
              key={i}
              className="text-[11px] font-bold uppercase tracking-wider px-3 py-1.5 bg-borderStrong text-textPrimary rounded-md"
            >
              {feat}
            </span>
          ))}
  ...
  const participantsBoxContent = (
    <>
      ...
      {data.participants && data.participants.length > 0 && (
        <div className="mb-6">
          <span className="neo-label-sm mb-2 block">
          ...
          <div className="max-h-32 overflow-y-auto p-3 bg-bgElevated border border-borderSubtle rounded-lg flex flex-wrap gap-1.5 custom-scrollbar">
            {data.participants.map((p, i) => (
              <span
                key={i}
                className="text-[10px] font-mono text-textSecondary bg-bgBase border border-borderStrong px-1.5 py-0.5 rounded"
              >
                @{p}
              </span>
            ))}
      ...
      <p className="text-xs text-textSecondary leading-snug">
        <strong className="text-textPrimary">Note:</strong> The participant
        count shown here may be lower than the public count displayed on X. X
        often inflates its numbers by including spam bots, suspended users, and
        private accounts. Our system filters these out automatically,
        guaranteeing a 100% fair draw among valid, public human participants.
      </p>
    </>
  );
    ...
    <div className="max-w-4xl mx-auto animate-fade-in-up">
      ...
      {pastDraws.length > 0 && (
        <div className="mb-8 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-amber-700 dark:text-amber-400 font-medium text-sm flex items-start gap-3">
          ...
          <div>
            <strong>Notice for Participants:</strong> This post has been drawn{" "}
            {pastDraws.length} time(s) previously. Scroll down to verify past
            draw history.
          </div>
        </div>
      )}
      ...
      <div className="mb-10 text-center flex flex-col items-center">
        ...
        <p className="mt-2 text-sm text-textSecondary">
          This draw has been finalized. Results are permanently recorded and
          cannot be altered.
        </p>
      </div>
          ...
          <div className="neo-card p-8 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sm font-semibold text-accentPrimary hover:underline break-all"
                >
                  {data.tweetId}
                </a>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  {data.hostUsername &&
                    !["unknown", "Anonymous", "simulated_user"].includes(
                      data.hostUsername,
                    ) && (
                      <Avatar
            ...
            <div className="pt-6 border-t border-borderSubtle">
              <div className="text-xs font-mono text-textSecondary flex flex-col sm:flex-row sm:items-center gap-2">
                <strong className="text-textPrimary font-sans shrink-0">
                  Draw ID:
                </strong>
                <CopyableText
                  text={drawId}
          ...
          <div className="neo-card p-8 h-fit">
            <div className="flex items-center justify-between mb-6 border-b border-borderSubtle pb-4">
              ...
              <span className="text-xs uppercase font-bold px-3 py-1.5 rounded-full bg-accentPrimary/15 text-accentPrimary">
                {data.winners.filter((w) => w.status !== "failed").length} total
              </span>
            </div>
                ...
                >
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <span className="text-xs font-semibold text-accentPrimary flex items-center gap-1.5 mt-1.5 uppercase tracking-wide">
                      <span className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-accentPrimary/20 text-[10px]">
                        {i + 1}
                      </span>
                      Primary Winner
                    </span>
                ...
                >
                  ...
                  <div className="flex flex-col flex-1">
                    ...
                    <span className="text-[11px] font-semibold text-textMuted flex items-center gap-1.5 mt-1.5 uppercase tracking-wide">
                      <span className="inline-flex h-4 w-4 items-center justify-center rounded-full border border-borderStrong text-[10px]">
              ...
              >
                ...
                <div className="flex flex-col flex-1">
                  ...
                  <div className="mt-2 text-xs font-medium text-red-500/80 bg-red-500/10 px-2 py-1.5 rounded-md inline-block">
                    <span className="block text-[10px] uppercase tracking-wider mb-0.5 opacity-80">
      ...
      <div className="neo-card p-8 mb-8 text-center border-accentPrimary/30 bg-accentPrimary/5">
        ...
        <p className="text-sm text-textSecondary mb-6 max-w-xl mx-auto">
          If you found this tool helpful for running your giveaway, consider
          leaving a tip. Your support keeps the platform provably fair and free
          for everyone!
        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <div className="flex flex-col items-center p-4 bg-bgBase border border-borderStrong rounded-xl w-full sm:w-auto hover:border-accentPrimary/50 transition-colors">
            <span className="text-xs font-bold uppercase tracking-wider text-textMuted mb-2">
              EVM Address
            </span>
            <CopyableText
              text="0x6e9b40a8fe85e7dcff40cfc9aa526106fe8e0546"
              className="text-xs font-mono text-accentPrimary"
            />
          </div>
          <div className="flex flex-col items-center p-4 bg-bgBase border border-borderStrong rounded-xl w-full sm:w-auto hover:border-accentPrimary/50 transition-colors">
            <span className="text-xs font-bold uppercase tracking-wider text-textMuted mb-2">
            ...
            <CopyableText
              text="AnBiWNPW68djMF6ERBpueF8tWmcvHr6iCYzriXwGh9k6"
              className="text-xs font-mono text-accentPrimary"
            />
          </div>
        </div>
      ...
      {pastDraws.length > 0 && (
        <div className="neo-card p-8 mt-8 border-amber-500/30">
          <h3 className="text-xl font-bold text-textPrimary border-b border-borderSubtle pb-4 flex items-center gap-2 mb-6">
            <FaClockRotateLeft className="text-amber-500" /> Past Draw Logs for
            this Post
          </h3>
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-bold text-textPrimary">
                    Draw: {past._id.slice(0, 8)}
                  </span>
                  <span className="text-xs text-textSecondary">
                    {new Date(past.createdAt).toLocaleString()}
                  </span>
                </div>
